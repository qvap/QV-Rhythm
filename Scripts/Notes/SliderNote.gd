...
var SHOULD_DELETE_ENDPOINT: bool = false
...
func _process(delta: float) -> void:
	...
	LINE_NODE.points[1] = Vector2(NEXT_ROAD.global_position.x - global_position.x, 0.0 - (NOTE_SPEED * (Conductor.s_per_quarter *\
	...
	if SLIDING:
		...
		var m : float = 0.0 - (NOTE_SPEED * (Conductor.s_per_quarter * (NEXT_NOTE_SPAWN_QUARTER - SPAWN_QUARTERS)))
...
func miss_note() -> void:
	match NOTE_TYPE:
		Global.CONTROL_TYPE.SLIDERCONTROL:
			SLIDER_TO_CONTROL.LINE_NODE.modulate.a = 0.5
			SLIDER_TO_CONTROL.modulate.a = 0.5
			queue_free()
